...
## Work a precision-weighted update

A prediction and an observation disagree. The posterior is the precision-weighted
average, where precision π is inverse variance:

```
prior:        μ_p = 10.0,  σ_p = 1.0   →  π_p = 1/1.0² = 1.00
observation:  μ_o = 16.0,  σ_o = 2.0   →  π_o = 1/2.0² = 0.25

posterior μ = (10.0×1.00 + 16.0×0.25) / (1.00 + 0.25)
            = (10.00 + 4.00) / 1.25
            = 11.2
```

The observation was 6 units away and moved the estimate by **1.2**.

---

## Now make the senses trustworthy

Same disagreement, but σ_o = 0.5 instead of 2.0, so π_o = 4.00:

```
posterior μ = (10.0×1.00 + 16.0×4.00) / (1.00 + 4.00)
            = (10.00 + 64.00) / 5.00
            = 14.8
```

**Identical prediction error. Identical observation. The estimate moves 4.8
instead of 1.2**, because the precision changed.

---

## The moral

Precision is not a detail of the machinery. It is the **only** parameter that
decides whether your model updates on the world or overrules it.

Which is why the same framework predicts both attention — deliberately raising
the precision of one channel — and hallucination, where a prior's precision is
high enough that input cannot correct it.
